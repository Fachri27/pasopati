...
{{-- HEADER: judul asli dari kartu API (bukan dummy) --}}
...
{{-- EMPTY STATE: belum ada laporan — tanpa konten dummy --}}
<section class="pb-20">
    <div class="max-w-[640px] mx-auto px-5 text-center">
        ...
        <h2 class="font-display font-bold text-[clamp(1.4rem,3vw,1.9rem)] leading-[1.2] tracking-[-.01em] mb-3">
            {{ $isId ? 'Belum ada laporan untuk kasus ini' : 'No reports for this case yet' }}
        </h2>
        ...
        <a href="{{ $archiveUrl }}" class="inline-flex items-center gap-2 text-sm font-semibold text-forest hover:text-forest-d transition-colors">
            ...
            {{ $isId ? 'Kembali ke daftar Deforestory' : 'Back to Deforestory list' }}
        </a>
    </div>
</section>

{{-- BERLANGGANAN: elemen fungsional, bukan konten dummy --}}
