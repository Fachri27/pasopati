...
<div x-data="{ selectedId: {{ $fellowship->kategoris->first() ? (int)$fellowship->kategoris->first()->id : 'null' }} }"
    ...
    <aside class="md:w-64 w-full bg-white md:sticky md:top-0  md:self-start md:p-4 pt-5 px-3">
        <nav class="flex flex-col space-y-1">
            ...
            @endphp
            <button @click="selectedId = {{ (int)$category->id }}" :class="selectedId === {{ (int)$category->id }} 
    ...
    <main class="flex-1 px-5 py-3 md:py-2 text-sm">
        ...
        <div x-show="selectedId === {{ (int)$category->id }}" :key="{{ (int)$category->id }}"
