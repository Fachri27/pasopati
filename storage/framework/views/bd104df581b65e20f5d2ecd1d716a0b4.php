...
<section
    x-data="{
        focused: false,
        replyTo: <?php if ((object) ('replyingTo') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('replyingTo'->value()); ?>')<?php echo e('replyingTo'->hasModifier('live') ? '.live' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('replyingTo'); ?>')<?php endif; ?>,
        bodyDraft: <?php if ((object) ('body') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('body'->value()); ?>')<?php echo e('body'->hasModifier('live') ? '.live' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('body'); ?>')<?php endif; ?>,
        replyBodyDraft: <?php if ((object) ('replyBody') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('replyBody'->value()); ?>')<?php echo e('replyBody'->hasModifier('live') ? '.live' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('replyBody'); ?>')<?php endif; ?>,
        nameDraft: <?php if ((object) ('commentName') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('commentName'->value()); ?>')<?php echo e('commentName'->hasModifier('live') ? '.live' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('commentName'); ?>')<?php endif; ?>,
        replyNameDraft: <?php if ((object) ('replyName') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('replyName'->value()); ?>')<?php echo e('replyName'->hasModifier('live') ? '.live' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('replyName'); ?>')<?php endif; ?>,
        captchaReady: <?php echo e($isGuest ? 'false' : 'true'); ?>,
        replyCaptchaReady: <?php echo e($isGuest ? 'false' : 'true'); ?>,
        openThreads: <?php echo \Illuminate\Support\Js::from($openThreads)->toHtml() ?>,
        resizeTick: 0,
        ...
        captchaOn: <?php echo e(($isGuest && $siteKey) ? 'true' : 'false'); ?>,
        init() {
            if (this.captchaOn) {
                this.widgetId = null;
        ...
        fmt(type) {
            ...
            if (type === 'bold') { document.execCommand('bold'); }
                ...
                const placeholder = <?php echo \Illuminate\Support\Js::from($locale === 'id' ? 'teks tautan' : 'link text')->toHtml() ?>;
...
>
    ...
    <div class="flex items-end justify-between gap-5">
        <div>
            <p class="text-[10px] font-bold uppercase tracking-[0.18em] text-[#2B5343]"><?php echo e($t['diskusi']); ?></p>
            <h2 class="mt-2 text-2xl font-black tracking-[-0.035em]"><?php echo e($t['komentar']); ?></h2>
        </div>
        <p class="text-xs text-auriga-muted"><?php echo e($totalComments); ?> <?php echo e($t['komentar_count']); ?></p>
    </div>
...
unset($__errorArgs, $__bag); ?><!--[if ENDBLOCK]><![endif]-->
            ...
            <div x-show="!(nameDraft || '').trim()" x-cloak>
                <div class="mt-4 flex items-center gap-3" aria-hidden="true">
                    ...
                    <span class="text-[10px] font-bold uppercase tracking-[0.18em] text-auriga-muted"><?php echo e($t['atau']); ?></span>
                    <span class="h-px flex-1 bg-[#e2d8cc]"></span>
                </div>
                <div class="mt-4 border border-[#e2d8cc] bg-[#f8f6f2] p-5 text-center">
                    <p class="text-sm text-[#6f665c]"><?php echo e($t['google_login']); ?></p>
                    <a href="<?php echo e(route('comment.google.login')); ?>?intended=<?php echo e(urlencode(url()->current())); ?>"
        ...
        <?php else: ?>
            ...
            ?>
            <div class="flex items-center gap-3 px-1">
                <!--[if BLOCK]><![endif]--><?php if($authAvatar): ?>
                    <img src="<?php echo e($authAvatar); ?>" alt="" class="h-11 w-11 shrink-0 rounded-full object-cover">
                <?php else: ?>
                    <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[#2B5343] text-base font-semibold uppercase text-white" aria-hidden="true"><?php echo e(strtoupper(mb_substr($authUser->name, 0, 1))); ?></div>
                <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
                <p class="text-xs text-auriga-muted">
                    <?php echo e($t['login_sebagai']); ?> <span class="font-semibold text-auriga-ink"><?php echo e($authUser->name); ?></span>
                    <a href="<?php echo e(route('comment.logout')); ?>?intended=<?php echo e(urlencode(url()->current())); ?>" class="ml-2 font-medium text-[#2B5343] hover:underline"><?php echo e($t['keluar']); ?></a>
                </p>
            </div>
        <?php endif; ?><!--[if ENDBLOCK]><![endif]-->

        <div class="mt-4 overflow-hidden bg-[#f5f5f5] transition-all duration-300 focus-within:bg-[#fafafa]">
            <label for="comment-message" class="sr-only"><?php echo e($t['komentar']); ?></label>
            <div id="ta-root" wire:ignore contenteditable="true" role="textbox" aria-multiline="true" aria-label="<?php echo e($t['komentar']); ?>" data-placeholder="<?php echo e($t['placeholder']); ?>" x-on:focus="focused = true; $nextTick(() => nudgeCaptcha())" x-on:input="syncEditor($el)" :class="focused ? 'min-h-28' : 'min-h-16'" class="ce-input block w-full whitespace-pre-wrap break-words border-0 bg-transparent px-5 py-4 text-sm leading-7 outline-none transition-all duration-300 focus:ring-0 sm:text-base"></div>
            <!--[if BLOCK]><![endif]--><?php $__errorArgs = ['body'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> <p class="px-5 pb-3 text-xs text-auriga-red"><?php echo e($message); ?></p> <?php unset($message);
if (isset($__messageOriginal)) { $message = $__messageOriginal; }
endif;
unset($__errorArgs, $__bag); ?><!--[if ENDBLOCK]><![endif]-->
            ...
            <!--[if BLOCK]><![endif]--><?php if($isGuest && $siteKey): ?>
                <div x-cloak x-show="focused" class="border-t border-black/5 px-5 py-4">
                    ...
                    <div id="captcha-root" wire:ignore class="min-h-[65px]"><span class="text-xs text-auriga-muted"><?php echo e($t['captcha_load']); ?></span></div>
...
unset($__errorArgs, $__bag); ?><!--[if ENDBLOCK]><![endif]-->
            ...
            <div x-cloak x-show="focused" class="flex items-center justify-between gap-4 border-t border-black/5 px-5 py-4">
                ...
                <div class="flex items-center gap-3">
                    <button type="button" x-on:click="focused = false" class="px-3 py-2 text-xs font-medium text-auriga-ink transition hover:text-[#2B5343]"><?php echo e($t['batal']); ?></button>
                    <button type="submit" :disabled="!canPost" :class="canPost ? 'bg-[#2B5343] enabled:hover:bg-[#1f3d31]' : 'bg-[#2B5343] cursor-not-allowed opacity-35'" class="rounded-full px-5 py-2.5 text-[10px] font-bold uppercase tracking-[0.1em] text-white transition"><?php echo e($t['kirim']); ?></button>
                </div>
            </div>
        </div>

        <p class="mt-3 min-h-5 px-1 text-xs text-auriga-muted"><?php echo e($t['moderasi']); ?></p>
    </form>
    ...
    <div class="comment-list mt-9 space-y-7">
        ...
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
            <p class="text-sm text-auriga-muted"><?php echo e($t['empty']); ?></p>
        <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
    </div>
    ...
    <?php $__env->startPush('scripts'); ?>
        ...
        <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
    <?php $__env->stopPush(); ?>
</section><?php /**PATH /Users/aiti/pasopati/resources/views/livewire/comment-section.blade.php ENDPATH**/ ?>